...
**Date:** February 2026
**Status:** All 6 phases complete; Deep AI Insights & Evidence-Based Risk Scoring shipped (v1.1a)
...
| **014** `ai_discussions.sql` | 3 | `ai_discussions`, `ai_discussion_messages` for multi-turn AI chat |
| **015** `cohort_v2.sql` | 5 | `cohort_snapshots`, `mv_patient_cohort_stats`, filter_version/color/is_pinned columns |
| **017** `deep_insights.sql` | Post-v1.1 | `structured_findings` JSONB + `clinical_trajectory` on `patient_ai_insights`; `patient_risk_history` table for longitudinal tracking; `idx_daily_entries_patient_submitted` performance index |
...
| `GET` | `/insights/:id/risk-history` | Post-v1.1 | Clinician (care-team gated) |
...
| `GET /insights/:id/ai` | Add `structured_findings`, `clinical_trajectory`, `risk_history` to response (v1.1a) |
| `POST /insights/:id/ai/trigger` | Accept `nightly_deep_analysis` type with automatic `period_days: 30` (v1.1a) |
