...
## Post-v1.1 — Deep AI Insights & Evidence-Based Risk Scoring (v1.1a)

### Database — migration 017 (deep insights)

- [x] Create `packages/db/migrations/017_deep_insights.sql`
- [x] Add `structured_findings JSONB` and `clinical_trajectory TEXT` columns to `patient_ai_insights`
- [x] Expand `insight_type` CHECK to include `'nightly_deep_analysis'`
- [x] Create `patient_risk_history` table (longitudinal risk score tracking)
- [x] Add performance index `idx_daily_entries_patient_submitted` on `daily_entries (patient_id, submitted_at DESC)`
- [x] Apply migration via psql and verify

### Risk scoring — graduated 10-rule rewrite

- [x] Rewrite `apps/api/src/services/riskScoring.ts` with 10 graduated rules
- [x] Extend `RiskFactor` interface with `domain`, `contribution`, `detail` fields
- [x] Add `RiskDomain` type: `'safety' | 'mood' | 'engagement' | 'physical' | 'medication'`
- [x] Implement over-allocation design (132 max raw, capped at 100)
- [x] Implement recency decay for C-SSRS scoring (48h=1.0×, 7d=0.8×, 14d=0.6×)
- [x] Implement PHQ-9 trajectory bonus (+5 for ≥5pt increase from prior)
- [x] Implement consecutive mood streak graduation (3d→10, 5d→13, 7d→15)
- [x] Add 3 new rules: R08 Sleep Disruption, R09 GAD-7 Anxiety, R10 PHQ-9 Item 9 SI
- [x] Dual-write `persistRiskScore` to both `patients` table AND `patient_risk_history`

### AI insights worker — enriched snapshot & deep analysis

- [x] Add `buildEnrichedClinicalSnapshot()` with ~12 parallel SQL queries
- [x] Add assessment trajectory queries (PHQ-9, GAD-7, ASRM with deltas)
- [x] Add sleep pattern analysis (7d avg, variability, quality, short nights, trend)
- [x] Add passive health summary (steps, HRV, resting HR, trends)
- [x] Add medication adherence detail (per-medication rates, longest miss streak)
- [x] Add social trend analysis (avg social score, avoidance days, trend)
- [x] Add cross-domain correlations via PostgreSQL `CORR()`
- [x] Add `buildDeepAnalysisPrompt()` for structured JSON output from LLM
- [x] Add `nightly_deep_analysis` job type to worker process handler
- [x] Store `structured_findings` JSONB and `clinical_trajectory` in INSERT

### Nightly scheduler — bug fix & deep analysis fan-out

- [x] Fix Ollama gate bug: change `&&` to `||` for `config.aiProvider === 'ollama'` check
- [x] Add Step 5B: fan-out `nightly_deep_analysis` jobs for consented patients with new data
- [x] Implement efficient `EXISTS` subquery against `patient_ai_insights` to skip patients with no new data

### Insights routes — new endpoint & updated responses

- [x] Add `nightly_deep_analysis` to `AiHistoryQuerySchema` insight type enum
- [x] Add `RiskHistoryQuerySchema` for new endpoint validation
- [x] Update `GET /:patientId/ai` to include `structured_findings`, `clinical_trajectory`, `risk_history`
- [x] Add `GET /:patientId/risk-history` endpoint (care-team gated, 90-day default)
- [x] Update trigger endpoint to accept `nightly_deep_analysis` as allowed type

### Web UI — PatientDetailPage AI Insights tab redesign

- [x] Add types: `StructuredFindings`, `RiskHistoryPoint`, `RiskDomain`, `RiskFactorItem`
- [x] Add `DOMAIN_META` and `TRAJECTORY_META` constants
- [x] Create `RiskGaugeArc` component (SVG semicircular arc gauge, 4 color segments, needle, delta)
- [x] Create `RiskFactorBars` component (domain-grouped collapsible sections, graduated bars)
- [x] Create `TrajectorySparklines` component (SVG polyline mini-charts for risk/PHQ-9/GAD-7)
- [x] Create `EarlyWarningSignals` component (urgency-sorted warning boxes)
- [x] Create `DeepInsightPanel` component (structured findings display with legacy fallback)
- [x] Rewrite `AiInsightsTab` with two-column layout (480px left / flex right)
- [x] Add generate type dropdown (Deep Analysis / Weekly Summary)
- [x] Add insight history timeline with collapsible entries
- [x] Verify backward compatibility for older insights without `structured_findings`

### Verification

- [x] `npm run typecheck` — 0 errors across all workspaces
- [x] `npm run build` — all packages compile cleanly
- [x] Migration 017 applied and recorded in `_migrations`
- [x] Auto-deploy daemon detected changes and rebuilt production services
- [x] Worker running with `nightly_deep_analysis` job type registered
